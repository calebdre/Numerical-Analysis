...
def main(methods, examples, will_generate_plot, should_plot_solution, plot_type):
    ...
    for ivp in ivps:
        ...
        input_to_func_map = {
            ...
            "rk4":("Runge-Kutta 4th Order", rk4, 1),
            "ab4":("Adams-Bashforth 4-Step Explicit", ab_four_step_explicit, 4),
        ...
        for method in methods:
            if method in input_to_func_map.keys():
                ...
                results, exact = run_iterations(ivp, method_name, func, list(iteration_values), arg_num)
                # for result in results:
                if will_generate_plot:
                    if plot_type == "error":
                        plot_vals = [ex - estimate for ex, estimate in zip(exact, results)]
                    else:
                        plot_vals = [estimate for estimate in result]
                    iter_values_with_min = list(iteration_values[1:])
